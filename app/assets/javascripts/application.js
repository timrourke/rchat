...
$(document).on('ready', function() {
	...
	var loggedInUsers = window.loggedInUsers || [];

	if (user) {
		var fullLocation = location.hostname+(location.port ? ':'+location.port: '');
		var dispatcher = new WebSocketRails(fullLocation + '/websocket');
		...
		channel.bind('client_disconnected', function(message) {
			...
			if (Array.isArray(json)) {
				...
				for (var index in loggedInUsers) {
					chatUsers += '<li>' + json[0].username + '</li>';
					if (json != loggedInUsers[index].username && loggedInUsers[index].username != user.username) {
						checkDisconnection(loggedInUsers[index]);
					}	
				}
			}
